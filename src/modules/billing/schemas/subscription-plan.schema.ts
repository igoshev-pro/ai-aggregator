...
export class FreeModelAccessSchema {
  ...
  /**
   * 🆕 Опциональный фильтр по параметрам генерации.
   * Если задан — модель бесплатна ТОЛЬКО при совпадении этих параметров.
   * Сравнение нестрогое (== для number/string/boolean).
   *
   * Пример: { mode: 'draft' } → midjourney бесплатен только в обычном режиме,
   * fast/turbo идут платно.
   *
   * type: Object — позволяет хранить произвольный JSON-объект
   * (Mongoose иначе вырезал бы поле при .save() из-за strict mode).
   */

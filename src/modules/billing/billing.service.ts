...
export class BillingService implements OnApplicationBootstrap {
  ...
  /**
   * Унифицированный билдер preview для текстовых и медиа-моделей.
   * Используется в getModelPreviewCost и getModelPricing.
   */
  ...
  /**
   * Расширенный баланс пользователя со всеми составляющими + подписка + дневной расход.
   * Используется BillingController.getBalance.
   */
  async getBalance(userId: string) {
    ...
    if (activeSubscription) {
      ...
      subscriptionData = {
        ...
        features: (activeSubscription as any).features,
  ...
  /**
   * Полная информация о ценах модели:
   * preview + pricingMatrix + uiParameters + базовые поля.
   * Используется AiProvidersController.getModelPricing.
   */
  async getModelPricing(modelSlug: string) {
    ...
    return {
      ...
      pricingMatrix: (model as any).pricingMatrix || [],
      uiParameters: (model as any).uiParameters || [],
      inputCapabilities: (model as any).inputCapabilities || {
        acceptsImages: false,
        maxInputImages: 0,
      },
      limits: (model as any).limits || {},
      defaultParams: (model as any).defaultParams || {},
    };
  }

  /**
   * Оценка стоимости генерации с конкретными параметрами (без списания).
   * Тонкая обёртка над calculateGenerationCost.
   * Используется AiProvidersController.estimateCost.
   */
  ...
  /**
   * Статистика по доходам за последние N дней.
   * Группировки: по дням / моделям / новые подписки.
   * Используется AdminService.getRevenueAnalytics.
   */
  ...
  /**
   * Админская корректировка баланса (плюс/минус).
   * Используется AdminService для ручных правок.
   */

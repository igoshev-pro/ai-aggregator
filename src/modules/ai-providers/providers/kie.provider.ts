...
/**
 * Resolves a voice display name to an ElevenLabs voice_id.
 * If the value is already a voice_id (not found as a key), returns it as-is.
 */
function resolveVoiceId(voiceNameOrId: string): string {
  if (ELEVENLABS_VOICE_MAP[voiceNameOrId]) {
    return ELEVENLABS_VOICE_MAP[voiceNameOrId];
  }
  const lower = voiceNameOrId.toLowerCase();
  for (const [name, id] of Object.entries(ELEVENLABS_VOICE_MAP)) {
      ...
      if (elevenLabsModels.has(modelId)) {
        ...
        switch (modelId) {
          ...
          case 'elevenlabs/text-to-dialogue-v3': {
            ...
            // Map voice display names → ElevenLabs voice IDs
            const mappedDialogue = r.dialogue.map((line: any) => ({
              text: line.text,
              voice: resolveVoiceId(line.voice || 'Rachel'),
            }));

            this.logger.debug(
              `Dialogue voice mapping: ${r.dialogue.map((l: any) => l.voice).join(', ')} → ${mappedDialogue.map((l: any) => l.voice).join(', ')}`,
            );

            input.dialogue = mappedDialogue;
          ...
          case 'elevenlabs/text-to-speech-turbo-2-5': {
            ...
            input.text = textValue;
            // Resolve voice name → voice ID
            input.voice = resolveVoiceId(voiceValue);
            input.stability = stabilityValue;
